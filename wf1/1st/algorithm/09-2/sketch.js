...
// 1. グローバル変数・設定値エリア
// =========================================================
let scene = 'title'; // 画面ステート（'title', 'howto', 'game', 'result', 'gameover'）
let btnHowto; // 「？」ボタン用スプライト
let btnClose; // 「×」ボタン用スプライト
let player;
let coins = [];
let score = 0;
...
// 3. タイトル画面（title）
// =========================================================
// 【初期化】タイトル遷移瞬間に1回だけ実行される準備関数
function initTitle() {
    allSprites.removeAll();

    // 「？」ボタンのスプライト生成
    btnHowto = new Sprite(width - 30, 30, 40, 40, 'static');
    btnHowto.color = 'white';
    btnHowto.text = '？';
    btnHowto.textSize = 24;
}
...
// 8. 遊び方の説明画面（howto）
// =========================================================
// 【初期化】遊び方画面遷移瞬間に1回だけ実行される準備関数
function initHowto() {
    allSprites.removeAll();

    // 「×」ボタンのスプライト生成
    btnClose = new Sprite(width - 30, 30, 40, 40, 'static');
    btnClose.color = 'white';
    btnClose.textColor = 'red';
    btnClose.text = '×';
...
function drawHowto() {
    ...
    textAlign(CENTER);
    fill('#000');
    text('遊び方', width / 2, height / 2 - 60);

    textSize(18);
    text('矢印キーでプレイヤー（🐷）を操作します。', width / 2, height / 2 - 10);
    text('制限時間内にコインを全部集めてね！', width / 2, height / 2 + 20);

    // 「×」ボタンのクリック判定
    if (btnClose.mouse.presses()) {
        initTitle();
        scene = 'title';
    }
}